...
class MapGenerator:
    ...
    def plot(self):
        ...
        def prepare_map() -> np.ndarray:
            ...
            for grid_square in self.grid_squares:
                zvals[grid_square.y - min_y + EDGE_BUFFER + extra_in_height][grid_square.x - min_x + EDGE_BUFFER + extra_in_width] = 1
            zvals[STARTING_Y - min_y + EDGE_BUFFER + extra_in_height][STARTING_X - min_x + EDGE_BUFFER + extra_in_width] = 2
            return zvals
        ...
        update_title()
        BUTTON_HEIGHT = 0.1
        axnew = plt.axes([0.85, 0.45, 0.1, 0.1])
    ...
    def generate(self):
        ...
        for idx in range(main_path_length):
    ...
    def statistics(self, num_generations: int = 1000):
        ...
        print(f"Mean: {statistics.mean(floor_areas)}, mode: {statistics.mode(floor_areas)}, median: {statistics.median(floor_areas)}, standard deviation: {statistics.stdev(floor_areas)}")
        ax1.grid(zorder=0)
    ...
    def _create_branch(self, start: GridSquare, branch_direction: Direction):
        ...
        for idx in range(branch_length):
            branch_current_gridsquare = self._create_gridsquare_in_direction(branch_current_gridsquare, branch_direction)
    ...
    def _create_room(self, center: GridSquare, minW: int = MIN_ROOM_WIDTH, maxW: int = MAX_ROOM_WIDTH, minH: int = MIN_ROOM_HEIGHT, maxH: int = MAX_ROOM_HEIGHT):
        logger.info(f"Creating room at [{center.x}, {center.y}]")
        room_width = np.random.randint(minW, maxW + 1)
        room_height = np.random.randint(minH, maxH + 1)
    ...
    def _shift_sideways_one(self, current_grid_square: GridSquare, current_direction: Direction) -> GridSquare:
            return self._create_gridsquare_in_direction(current_grid_square, np.random.choice(turn_90_deg_options[current_direction]))
